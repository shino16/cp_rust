	...
	LazySegmentTree<On, Act, Apply>
{
	...
	pub fn new_from_slice(slice: &[On::Item], on_alg: On, act_alg: Act, apply: Apply) -> Self {
		let len = slice.len();
	...
	fn flush(&mut self, p: usize) {
		...
		for s in (0..r).rev() {
			let p = p >> s;
			self.apply(p, self.data[p >> 1].1);
			self.apply(p ^ 1, self.data[p >> 1].1);
			self.data[p >> 1].1 = self.act_alg.unit();
		}
	}
	fn build(&mut self, mut p: usize) {
		p >>= 1;
		while p != 0 {
			let data_pos = self.on_alg.op(self.data[p << 1].0, self.data[p << 1 | 1].0);
			self.data[p].0 = (self.apply)(data_pos, self.data[p].1);
			p >>= 1;
		}
	}
